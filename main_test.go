...
func TestCacheTTLRulesApplied(t *testing.T) {
	...
	var c1 cacheEntry
	if err := json.Unmarshal(b1, &c1); err != nil {
		t.Fatal(err)
	}
	...
	if err != nil {
		t.Fatalf("missing cache file 2: %v", err)
	}
	var c2 cacheEntry
	if err := json.Unmarshal(b2, &c2); err != nil {
		t.Fatal(err)
	}
...
func TestCacheTTLByExtension(t *testing.T) {
	...
	b2, err := os.ReadFile(p2)
	if err != nil {
		t.Fatalf("missing cache file robots: %v", err)
	}
	var c2 cacheEntry
	if err := json.Unmarshal(b2, &c2); err != nil {
		t.Fatal(err)
...
func TestCacheFilePathForURL(t *testing.T) {
	...
	if filepath.Dir(p1) != filepath.Join(dir, "b.com", "foo") {
		t.Fatalf("unexpected dir: %s", filepath.Dir(p1))
	}
	if p1 == p2 {
		t.Fatalf("expected different file names for different queries: %s == %s", p1, p2)
	}
	pNoQ, _ := cacheFilePathForURL(dir, "https://b.com/foo")
	if filepath.Base(pNoQ) != "index.json" {
		t.Fatalf("expected index.json for no-query, got %s", filepath.Base(pNoQ))
	}
}
